...
{
  *pNbContacts+=1;
  Repertoire[*pNbContacts-1]=*pNouveau;
}
...
int Rechercher(char NomRecherche[], struct sContact Repertoire[], 
               int NbContacts)
{
  for(int i=0;i<NbContacts;i++) if(!(strcmp(NomRecherche, Repertoire[i].Nom))) return i;
  return -1;
}
                   ...
                   char NomFichier[])
{
  ...
  if (fichier==NULL) perror(NomFichier);
  else
  {
...
int LireFichier1(struct sContact Repertoire[], char NomFichier[])
{
  ...
  while(fread(Repertoire+NbContacts,sizeof(struct sContact),1,fichier)) NbContacts++;
  fclose(fichier);
  return NbContacts;
}
...
int LireFichier2(struct sContact Repertoire[], char NomFichier[])
{
  ...
  NbContacts=ftell(fichier)/sizeof(struct sContact);
