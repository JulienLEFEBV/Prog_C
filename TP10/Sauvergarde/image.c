...
tImage ImLire(char NomFichier[])
{
    ...
    {  
        ...
        {
            fread(image->NivGris[i],sizeof(unsigned char),colonnes*sizeof(unsigned char),image_fichier);
        }
...
void ImEcrireBin(tImage Im, char NomFichier[])
{
    ...
    {
        fwrite(Im->NivGris[i], sizeof(unsigned char), Im->NbCol*sizeof(unsigned char), image_fichier);
    }
    fclose(image_fichier);
}
